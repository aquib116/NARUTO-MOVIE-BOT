    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("🔆彡[ CHRUNCHYROOL ]彡🔆", url=invite_link.invite_link)])
    ...
    if data.split("-", 1)[0] == "BATCH":
            ...
            try:
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          InlineKeyboardButton('🔆彡[ CHRUNCHYROOL ]彡🔆', url=f'https://telegram.me/{CHNL_LNK}'),
                         ]
                        ]
                    )
                )
        ...
        if is_valid == True:
            await message.reply_text(
                text=f"<b>Hey {message.from_user.mention}, You are successfully verified !\nNow you have unlimited access for all movies for 12hours.</b>",
                protect_content=False
            )
